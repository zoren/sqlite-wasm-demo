/*
  ...
  ***********************************************************************
  ...
  This was adapted from the demo-123.js file.
...
export const demo1 = async function (
  ...
  */
  try {
    ...
    log(
      "Query data with exec() using rowMode INTEGER (result column index)..."
    );
    db.exec({
    ...
    log(
      "Query data with exec() using rowMode $COLNAME (result column name)..."
    );
    db.exec({
    ...
    try {
      log(
        "The following use of the twice() UDF will",
